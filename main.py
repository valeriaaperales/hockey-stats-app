...
def main():
    ...
    for p in players_team1:
        if p[4].lower() == "yes" and p[1] and p[2]:
            tk.Button(player_frame1, text=f"{p[0]} - {p[1][0]}. {p[2]}", font=("Arial", 12, "bold")).pack(pady=7)
    tk.Button(player_frame1, text="Subs", font=("Arial", 12, "bold"), bg="#B0A9A9", command=lambda: sub.show_subs(root, 1)).pack(side="bottom", pady=10)
    ...
    for p in players_team2:
        if p[4].lower() == "yes" and p[1] and p[2]:
            tk.Button(player_frame2, text=f"{p[0]} - {p[1][0]}. {p[2]}", font=("Arial", 12, "bold")).pack(pady=7)
    tk.Button(player_frame2, text="Subs", font=("Arial", 12, "bold"), bg="#B0A9A9", command=lambda: sub.show_subs(root, 2)).pack(side="bottom", pady=10)
